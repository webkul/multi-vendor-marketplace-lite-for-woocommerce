...
$result = mb_convert_encoding( /* translators: %1$d: Order number, %2$s: Billing first name. */ wp_sprintf( esc_html__( 'Alas. Just to let you know — order #%1$d belonging to %2$s has been cancelled:', 'wk-marketplace' ), esc_html( $seller_order->get_order_number() ), esc_html( $seller_order->get_formatted_billing_full_name() ) ), 'UTF-8' ) . "\n\n" . 'Order #' . $seller_order->get_ID() . ' (' . $date_string . ') ' . "\n\n";
...
foreach ( $product_details as $product_id => $details ) {
	...
	for ( $i = 0; $i < $detail_c; ++$i ) {
		...
		if ( 0 === intval( $details[ $i ]['variable_id'] ) ) {
			$result .= mb_convert_encoding( $details[ $i ]['product_name'], 'UTF-8' ) . esc_html__( 'SKU: ', 'wk-marketplace' ) . $common_functions->wkmp_get_sku( $product ) . ' X ' . $details[ $i ]['qty'] . ' = ' . $seller_order->get_currency() . ' ' . $details[ $i ]['product_total_price'] . "\n\n";
		} else {
			...
			$result .= mb_convert_encoding( $details[ $i ]['product_name'], 'UTF-8' ) . ' (' . esc_html__( 'SKU: ', 'wk-marketplace' ) . $common_functions->wkmp_get_sku( $product ) . ' )';
