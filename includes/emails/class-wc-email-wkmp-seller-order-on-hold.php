...
if ( ! class_exists( 'WC_Email_WKMP_Seller_Order_On_Hold' ) ) {
	/**
	 * Class WC_Email_WKMP_Seller_Order_On_Hold
	 *
	...
	class WC_Email_WKMP_Seller_Order_On_Hold extends \WC_Email {
		 ...
		 *
		 * WC_Email_WKMP_Seller_Order_On_Hold constructor.
		 */
		public function __construct() {
			$this->id          = 'wkmp_seller_order_on_hold';
			$this->title       = esc_html__( 'Seller Order On-Hold', 'wk-marketplace' );
			$this->description = esc_html__( 'This is an order notification sent to sellers containing order details after an order is placed on-hold.', 'wk-marketplace' );

			$this->wkmp_default_email_place_holder();

			$this->template_html  = 'emails/wkmp-seller-order-on-hold.php';
			$this->template_plain = 'emails/plain/wkmp-seller-order-on-hold.php';
			$this->template_base  = WKMP_LITE_PLUGIN_FILE . 'woocommerce/templates/';

			add_action( 'wkmp_seller_order_on_hold_notification', array( $this, 'trigger' ), 10, 3 );
		...
		public function get_default_heading() {
			return __( 'Your order #{order_number} status moved to On-Hold.', 'wk-marketplace' );
		}
		...
		public function get_default_subject() {
			return __( 'Your [{site_title}]: Order #{order_number} status moved to On-Hold.', 'wk-marketplace' );
		}
	}
}
