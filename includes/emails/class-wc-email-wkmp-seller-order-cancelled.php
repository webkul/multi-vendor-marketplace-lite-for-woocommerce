...
if ( ! class_exists( 'WC_Email_WKMP_Seller_Order_Cancelled' ) ) {
	/**
	 * Class WC_Email_WKMP_Seller_Order_Cancelled
	 *
	...
	class WC_Email_WKMP_Seller_Order_Cancelled extends \WC_Email {
		 ...
		 *
		 * WC_Email_WKMP_Seller_Order_Cancelled constructor.
		 */
		public function __construct() {
			$this->id          = 'wkmp_seller_order_cancelled';
			$this->title       = esc_html__( 'Seller Order Cancelled', 'wk-marketplace' );
			$this->description = esc_html__( 'Cancelled order emails are sent to chosen recipient(s) when orders have been marked cancelled (if they were previously processing or on-hold).', 'wk-marketplace' );

			$this->wkmp_default_email_place_holder();

			$this->template_html  = 'emails/wkmp-seller-order-cancelled.php';
			$this->template_plain = 'emails/plain/wkmp-seller-order-cancelled.php';
			$this->template_base  = WKMP_LITE_PLUGIN_FILE . 'woocommerce/templates/';

			add_action( 'wkmp_seller_order_cancelled_notification', array( $this, 'trigger' ), 10, 3 );
		...
		public function get_default_heading() {
			return __( 'Your order #{order_number} has been cancelled.', 'wk-marketplace' );
		}
		...
		public function get_default_subject() {
			return __( 'Your [{site_title}]: Order #{order_number} has been cancelled.', 'wk-marketplace' );
		}
	}
}
